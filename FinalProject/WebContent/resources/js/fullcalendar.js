...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		var _element = this,
			...
			date = new Date(),
			viewName,  // the current view name (TODO: look into getting rid
						// of)
			...
			if (v != viewName) {
				...
				if (oldView) {
					...
					if (oldView.beforeHide) {
						oldView.beforeHide(); // called before changing
				...
				if (!newViewElement && view.afterShow) {
					view.afterShow(); // called after setting
										// min-height/overflow, so in final
										// scroll state (for Opera)
				}
		...
		function render(inc) {
			if (elementVisible()) {
				ignoreWindowResize++; // because view.renderEvents might
				...
				if (!view.start || inc || date < view.start || date >= view.end) {
					view.render(date, inc || 0); // responsible for clearing
													// events
					setSize(true);
		...
		// called when any event objects have been added/removed/changed,
		// rerenders
		...
		var publicMethods = {
			...
			updateEvent: function(event) { // update an existing event
				var i, len = events.length, e,
					...
					endDelta = event.end ?
						(event.end - (event._end || view.defaultEventEnd(event))) // event._end
																					// would
																					// be
																					// null
																					// if
			...
			rerenderEvents: eventsChanged, // TODO: think of renaming
											// eventsChanged
...
function Grid(element, options, methods, viewName) {
	...
	view = $.extend(this, viewMethods, methods, {
		...
		setHeight: setHeight,
		setWidth: setWidth,
		defaultEventEnd: function(event) { // calculates an end if event doesnt
											// have one, mostly for resizing
	...
	function dayClick(ev) {
		if (!view.option('selectable')) { // SelectionManager will worry about
											// dayClick
	...
	function renderDayOverlay(overlayStart, overlayEnd) { // overlayEnd is
															// exclusive
	...
	function _renderDayOverlay(row0, col0, row1, col1) { // row1,col1 is
															// inclusive
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0, rowI=0; rowI<rowCnt; rowI++) {
		...
		rowDivs[rowI] = getRow(rowI).find('td:first div.fc-day-content > div') // optimal
																				// selector?
...
function Agenda(element, options, methods, viewName) {
	...
	function renderDaySegs(segs, modifiedEventId) {
		if (options.allDaySlot) {
			...
			setHeight(viewHeight); // might have pushed the body down, so
									// resize
		}
	}
	...
	function draggableDayEvent(event, eventElement, isStart) {
		if (!options.disableDragging && eventElement.draggable) {
			...
			eventElement.draggable({
				zIndex: 9,
				opacity: view.option('dragOpacity', 'month'), // use whatever
																// the month
																// view was
																// using
				revertDuration: options.dragRevertDuration,
		...
		if (!options.disableDragging && eventElement.draggable) {
			...
			eventElement.draggable({
				...
				start: function(ev, ui) {
					...
					view.hideEvents(event, eventElement);
					if ($.browser.msie) {
						eventElement.find('span.fc-event-bg').hide(); // nested
																		// opacities
																		// mess
																		// up in
																		// IE,
				...
				stop: function(ev, ui) {
					...
					if (cell && (dayDelta || minuteDelta || allDay)) {
						...
						if ($.browser.msie) {
							eventElement
								.css('filter', '') // clear IE opacity
													// side-effects
								.find('span.fc-event-bg')
									.css('display', ''); // .show() made
															// display=inline
						}
						view.showEvents(event, eventElement);
					}
	...
	function renderSlotSelection(startDate, endDate) {
		...
		if (helperOption) {
			...
			if (col >= 0 && col < colCnt) { // only works when times are on same
											// day
				var rect = coordinateGrid.rect(0, col, 0, col, bodyContent); // only
																				// for
																				// horizontal
																				// coords
				...
				if (bottom > top) { // protect against selections that are
					...
					if ($.isFunction(helperOption)) {
						...
						if ($.browser.msie) {
							selectionHelper.find('span.fc-event-bg').hide(); // nested
																				// opacities
																				// mess
																				// up
																				// in
...
var viewMethods = {
	 ...
	 * defaultEventEnd(event) - render(events) - rerenderEvents()
	 * 
	 * 
	 * z-index reservations: 3 - day-overlay 8 - events 9 - dragging/resizing
	 * events
	 * 
	 */
	...
	eventEnd: function(event) {
		return event.end ? cloneDate(event.end) : this.defaultEventEnd(event); // TODO:
																				// make
																				// sure
																				// always
																				// using
	...
	reportEvents: function(events) { // events are already normalized at this
										// point
	...
	eventDrop: function(e, event, dayDelta, minuteDelta, allDay, ev, ui) {
		...
		view.trigger('eventDrop', e, event, dayDelta, minuteDelta, allDay, function() { // TODO:
																						// change
																						// docs
	...
	resizableDayEvent: function(event, eventElement, colWidth) {
		...
		if (!view.options.disableResizing && eventElement.resizable) {
			eventElement.resizable({
				...
				grid: colWidth,
				minWidth: colWidth/2, // need this or else IE throws errors
										// when too small
				containment: view.element.parent().parent(), // the main
																// element...
...
var parseTime = fc.parseTime = function(s) { // returns minutes since start
												// of day
